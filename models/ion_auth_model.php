...
class Ion_auth_model extends CI_Model
{
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$data = array(
			...
			'password'   => $password,
			'email'      => $email,
			'ip_address' => sprintf('%u', ip2long('0.0.0.0')),
			'created_on' => now(),
			'created_by' => sprintf('%u', ip2long($ip_address)),
			'last_login' => now(),
			'active'     => 1
		);
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['users'], array('last_login' => now(), 'ip_address' => sprintf('%u', ip2long($this->input->ip_address()))), array('id' => $id));
