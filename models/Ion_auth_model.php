...
class Ion_auth_model extends CI_Model
{
	...
	public function change_password($identity, $old, $new)
	{
		...
		// just added code given below for check Old and New Password should not be same
		
		$new_and_old_password_match = $this->hash_password_db($user->id, $new);
		
		if ($new_and_old_password_match === TRUE)
		{
			$this->set_error('password_match_old_unsuccessful');
			return FALSE;
		}
	...
	**/
	...
	public function old_password_check($code, $user_id, $pass)
	{
		...
		$res = $this->hash_password_db($user_id, $pass);
		// print_r($res);
		return $res;
