...
class IonAuthModel
{
	...
	{
		if (empty($email)) {
			$this->setError('IonAuth.empty_email_address');
			return FALSE;
		}

		if (!$this->emailCheck($email)) {
			$this->setError("IonAuth.unregistered_email");
			return FALSE;
		}
		...
		if ($user->active) {
			$this->setError("IonAuth.already_activated_email");
			return FALSE;
		}
		...
		if ($user) {
			...
			$data = [
				// $this->identityColumn => $user->identity, // Can't get it work
				'id'         		  => $user->id,
				'email'      		  => $user->email,
			...
			$email->clear();
			$email->setFrom($this->config->adminEmail, $this->config->siteTitle);
			$email->setTo($user->email);
			$email->setSubject($this->config->siteTitle . ' - ' . lang('IonAuth.emailActivation_subject'));
